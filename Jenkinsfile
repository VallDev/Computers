    ...
    stages {
        ...
        stage('Rebuilding the image from .tar format') {
            steps {
                ...
                sh "ssh -v -o StrictHostKeyChecking=no andres@192.168.0.10 'cd ImagesToRun && docker load -i computers-go.tar'"
            }
        }
        ...
        stage('Runing Docker Image') {
            steps {
                ...
                sh "ssh -v -o StrictHostKeyChecking=no andres@192.168.0.10 'cd ImagesToRun && docker run -d -p 8080:8080 --name computers-go-app -tid computers-go .'"
            }
        }
